...
export const findAllCases = async (): Promise<any[]> => {
  try {
    ...
    const [customerDocumentRows]: any = await dbQuery(`
      SELECT *
    ...
    const [historyRows]: any = await dbQuery(`SELECT h.*,fu.name AS performed_by_name FROM fcu_case_history h LEFT JOIN fcu_users fu ON fu.id=h.performed_by ORDER BY h.created_at DESC,h.id DESC`);
    ...
    const customerDocumentsByLead = new Map<string, any[]>();
    const customerDocumentsByUser = new Map<number, any[]>();
    for (const document of customerDocumentRows) {
      const leadId = String(document.lead_id || '').trim().toUpperCase();
      if (leadId) {
        if (!customerDocumentsByLead.has(leadId)) customerDocumentsByLead.set(leadId, []);
        customerDocumentsByLead.get(leadId)!.push(document);
      }
      if (document.user_id) {
        const uid = Number(document.user_id);
        if (!customerDocumentsByUser.has(uid)) customerDocumentsByUser.set(uid, []);
    ...
    for (const reference of referenceRows) {
      ...
      if (!referencesByUser.has(userId)) referencesByUser.set(userId, []);
    ...
    const cases = rows.map((row: any, index: number) => {
      ...
      const allDocsMap = new Map<number, any>();
