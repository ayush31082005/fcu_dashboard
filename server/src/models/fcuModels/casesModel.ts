...
export const findAllCases = async (): Promise<any[]> => {
  try {
    ...
    const [telecallerDocumentRows]: any = await dbQuery(`
    ...
    const telecallerDocumentsByLead = new Map<string, any[]>();
    for (const document of telecallerDocumentRows) {
      const leadId = String(document.lead_id || '').trim().toUpperCase();
      if (!leadId) continue;
      if (!telecallerDocumentsByLead.has(leadId)) telecallerDocumentsByLead.set(leadId, []);
    ...
    const cases = rows.map((row: any, index: number) => {
      ...
      const docs = (telecallerDocumentsByLead.get(caseLeadId) || []).map((document: any) => {
