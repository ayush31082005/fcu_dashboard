...
import { findMobileForUpiLookup, saveMobileUpiDetails } from '../../models/fcuModels/mobileUpiModel';

export const lookupMobileUpi = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const application = await findMobileForUpiLookup(applicationId);
    ...
    const clientRefNum = `fcu-${applicationId}-${randomUUID().slice(0, 8)}`;
