...
import { hasIncompleteDocumentRequest } from '../../models/fcuModels/documentRequestModel';
...
export const reviewDocument = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const validDocumentIds = (currentCase?.docs || []).map((document: any) => String(document.id));
    ...
    await addCaseHistory(applicationId, 'DOCUMENT_REVIEW', `${documentId.toUpperCase()} document ${status.toLowerCase()}`, status === 'REJECTED' ? `Reason: ${reason}` : `Document review status changed to ${status}.`, Number((req as any).fcuUser.id));
...
export const approveAllDocuments = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!(await ensureCaseOwner(req, res, applicationId))) return;
    const currentCase = (await findAllCases()).find(item => item.databaseId === applicationId);
    const documentIds = (currentCase?.docs || []).map((document: any) => String(document.id));
...
export const reviewEkycCheck = async (req: Request, res: Response): Promise<void> => {
  try {
    const applicationId = parseApplicationId(req.params.caseId);
    const checkId = String(req.params.checkId || '');
    const status = String(req.body.status || '').toUpperCase();
    if (!applicationId || !['identity','credit','bank'].includes(checkId) || !['PASS','FAIL','PENDING'].includes(status)) { res.status(400).json({ status:'error', message:'Invalid eKYC review' }); return; }
    if (!(await ensureCaseOwner(req, res, applicationId))) return;
    const userId = Number((req as any).fcuUser.id);
    await updateEkycReview(applicationId, checkId, status, userId);
    await addCaseHistory(applicationId, 'EKYC_REVIEW', `${checkId} check marked ${status}`, `eKYC review status changed to ${status}.`, userId);
    res.json({ status:'success', data:{ checkId, status } });
  } catch (error) { console.error('FCU eKYC review error:', error); res.status(500).json({ status:'error', message:'Unable to save eKYC review' }); }
};
...
export const performWorkflowAction = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    await saveWorkflowAction(applicationId, nextStage, caseStatus, reviewer.id, fieldAssignedTo, action === 'SEND_CREDIT' ? 3 : undefined, rejectionDecision, reason);
    const actionTitle: Record<string, string> = { APPROVE_CASE:'Case approved', REJECT_CASE:'Case rejected', FLAG_FRAUD:'Case permanently flagged as fraud', SEND_FIELD:'Sent to field verification', WAIVE_FIELD:'Field verification waived', SEND_CREDIT:'Sent to credit team', HOLD_CASE:'Case placed on hold', FORWARD_REJECT:'Forwarded for rejection' };
    ...
    await addCaseHistory(applicationId, 'WORKFLOW_ACTION', actionTitle[action] || action, actionDetail, reviewer.id);
    let whatsapp: { sent: boolean; message?: string } | null = null;
    if (action === 'REJECT_CASE' || action === 'FLAG_FRAUD') {
      try {
        await sendWhatsAppRejection(String(currentCase?.mobile || ''), String(currentCase?.borrower || 'Customer'));
        whatsapp = { sent: true };
