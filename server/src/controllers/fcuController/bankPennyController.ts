...
import { findBankForPennyVerification, saveBankPennyVerification } from '../../models/fcuModels/bankPennyModel';

const firstValue = (...values: any[]) => values.find(value => value !== undefined && value !== null && value !== '');

const findProviderValue = (value: any, keys: string[]): any => {
...
export const verifyBankPenny = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!apiId || !apiKey || !tokenId) {
      res.status(503).json({ status: 'error', message: 'Bank Penny API credentials are not configured' }); return;
    }
    ...
    await saveBankPennyVerification({
