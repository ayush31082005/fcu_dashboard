...
exports.searchCkyc = void 0;
const ckycModel_1 = require("../../models/fcuModels/ckycModel");
const pick = (...values) => values.find(v => v !== undefined && v !== null && v !== '');
const searchCkyc = async (req, res) => {
    try {
        ...
        const customer = await (0, ckycModel_1.findCaseForCkycSearch)(applicationId);
        if (!customer) {
        ...
        await (0, ckycModel_1.saveCkycSearch)({ applicationId, userId: Number(customer.user_id), panNumber: pan, ckycNumber: normalized.number || null,
        ...
        console.error('FCU CKYC search error:', { code: error?.cause?.code || error?.code, message: error?.cause?.message || error?.message });
        res.status(502).json({ status: 'error', message: error?.name === 'TimeoutError' ? 'CKYC provider timed out' : 'CKYC provider is unreachable' });
    }
};
exports.searchCkyc = searchCkyc;
