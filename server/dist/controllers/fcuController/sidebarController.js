...
const sidebarModel_1 = require("../../models/fcuModels/sidebarModel");
const browserFrom = (agent) => {
    if (/Edg\//i.test(agent))
...
const getSidebar = async (req, res) => {
    try {
        ...
        const latestLogin = data.activities.find((item) => item.action === 'login');
        ...
        const loginActivities = data.activities.filter((item) => item.action === 'login');
        ...
        if (loginActivities.length === 0)
            loginActivities.push(currentSessionLogin);
        res.json({
            status: 'success',
            data: {
                session: {
