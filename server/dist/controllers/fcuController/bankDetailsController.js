...
exports.updateBankDetails = void 0;
...
const updateBankDetails = async (req, res) => {
    try {
        ...
        const [appRows] = await db_1.default.query('SELECT id, user_id FROM applications WHERE id = ? LIMIT 1', [applicationId]);
        const app = appRows[0];
        if (!app) {
        ...
        const { accountHolderName = '', bankName = '', accountNumber = '', ifscCode = '', branchName = '', accountType = 'savings', salaryAccount = 'No', verificationStatus = 'Verified', } = req.body || {};
    ...
    `, [
        ...
        const reviewerId = Number(req.fcuUser?.id) || undefined;
        await (0, casesModel_1.addCaseHistory)(applicationId, 'EKYC_REVIEW', 'Bank Account Details Updated', `Bank details updated (Holder: ${accountHolderName || 'N/A'}, Bank: ${bankName || 'N/A'}, Acc: ${accountNumber || 'N/A'}, Status: ${verifiedVal})`, reviewerId);
        const updatedBank = {
