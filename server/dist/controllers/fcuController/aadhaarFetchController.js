...
exports.updateAadhaarRelation = exports.fetchAadhaarDetails = void 0;
const aadhaarFetchModel_1 = require("../../models/fcuModels/aadhaarFetchModel");
...
const fetchAadhaarDetails = async (req, res) => {
    try {
        ...
        if (!/^\d{8,16}$/.test(aadhaar)) {
            res.status(400).json({ status: 'error', message: 'Enter a valid Aadhaar number' });
            return;
        }
        const customer = await (0, aadhaarFetchModel_1.findCaseForAadhaarFetch)(applicationId);
        if (!customer) {
        ...
        const addressObj = data.address || data.address_details || {};
        ...
        if (!success) {
            res.status(502).json({ status: 'error', message: normalized.providerMessage || `Aadhaar provider returned HTTP ${response.status}` });
            return;
        }
        await (0, aadhaarFetchModel_1.saveAadhaarFetch)({ applicationId, userId: Number(customer.user_id), aadhaarNumber: String(normalized.aadhaarNumber),
...
const updateAadhaarRelation = async (req, res) => {
    try {
        ...
        if (!allowedRelations.has(relation)) {
            res.status(400).json({ status: 'error', message: 'Select a valid relation' });
            return;
        }
        const saved = await (0, aadhaarFetchModel_1.saveAadhaarRelation)(applicationId, relation);
        if (!saved) {
            res.status(409).json({ status: 'error', message: 'Fetch Aadhaar details before saving relation' });
            return;
        }
...
exports.updateAadhaarRelation = updateAadhaarRelation;
