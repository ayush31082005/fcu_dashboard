...
const getDashboardData = async () => {
  ...
  `);
    ...
    return {
        summary: {
            totalCases: total,
            assigned: dailyRows.reduce((sum, row) => sum + Number(row.assigned || 0), 0),
        ...
        dailyCases: dailyRows.map((row) => ({ day: row.day, assigned: Number(row.assigned), completed: Number(row.completed), pending: Number(row.assigned) - Number(row.completed) })),
