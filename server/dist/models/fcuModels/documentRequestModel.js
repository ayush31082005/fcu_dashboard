...
exports.findDocumentRequestRecipient = findDocumentRequestRecipient;
const findDocumentRequestByApplication = async (applicationId) => {
    const [rows] = await (0, dbQuery_1.dbQuery)(`
...
exports.findDocumentRequestByApplication = findDocumentRequestByApplication;
const findDocumentRequestByToken = async (token) => {
    const [rows] = await (0, dbQuery_1.dbQuery)(`
    SELECT r.id, r.application_id, r.token, r.status, r.expires_at, r.created_at,
...
exports.findDocumentRequestByToken = findDocumentRequestByToken;
const hasIncompleteDocumentRequest = async (applicationId) => {
    const [rows] = await (0, dbQuery_1.dbQuery)(`
    SELECT r.status,
  ...
  `, [applicationId]);
    ...
    if (!rows.length || Number(rows[0].total_count) === 0)
        return true;
    return rows[0].status !== 'COMPLETED' || Number(rows[0].pending_count) > 0;
};
exports.hasIncompleteDocumentRequest = hasIncompleteDocumentRequest;
const createDocumentRequestRecord = async (applicationId, token, userId, documents) => {
    const connection = await db_1.default.getConnection();
    try {
        await connection.beginTransaction();
        const [applications] = await connection.query('SELECT id FROM applications WHERE id = ? FOR UPDATE', [applicationId]);
        if (!applications.length) {
            const error = new Error('Application no longer exists in the database. Refresh the Applications page.');
...
exports.createDocumentRequestRecord = createDocumentRequestRecord;
const saveRequestedDocumentUpload = async (token, documentId, fileName, filePath) => {
    const [result] = await (0, dbQuery_1.dbQuery)(`
