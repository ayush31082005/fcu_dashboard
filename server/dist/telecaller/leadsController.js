...
exports.updateLeadStatus = exports.addLeadLog = exports.getLeadLogs = exports.getLeads = void 0;
    ...
    `);
        ...
        const leads = rows.map((row) => {
            ...
            if (row.application_status === 'approved')
                stage = "Follow Up";
            if (row.application_status === 'loan reject')
                stage = "Reject";
            if (!row.application_status)
                stage = "Follow Up"; // No application yet
            return {
...
const getLeadLogs = async (req, res) => {
    try {
        ...
        const [userRows] = await db_1.default.query('SELECT id FROM users WHERE id = ? OR lead_number = ? OR lead_number = ? LIMIT 1', [userId, `GP-LEAD-${userId}`, `USR-${userId}`]);
...
exports.getLeadLogs = getLeadLogs;
const addLeadLog = async (req, res) => {
    try {
        const { userId } = req.params;
